...
de 2026, y cada sección lleva la suya.
...
`git log` de antes de la mudanza sigue siendo válido. Lo que quedó allá: las
...
del navegador abierto, que es la trampa de `AnimatePresence` anotada en
`ESTADO.md`—: las dos tarjetas de caducidad con sus fallos, la transcripción de
...
de referencia que ya estaban escritos para esa base ($21.368.714,99 →
...
> `AnimatePresence mode="wait"` que está en `ESTADO.md`—. La salida fue una página
...
en `ESTADO.md`: ninguna de las 24 dice «Ver más».
...
- **En alimentos se dijo qué supuesto calculaba la app y cuál no.** El segundo
  ...
  implementó el mismo día, en la segunda tanda. Queda como ejemplo de que decir «esto no
...
## El sitio de herramientas pasó a dominio propio — 5/8

`javiercuneo.com.ar` quedó activo y las tres URL absolutas de `lib/enlaces.ts`
pasaron de `javiercuneo.github.io/herramientas-judiciales/` al dominio. Fue
exactamente el barrido para el que ese archivo se centralizó: una lectura y
listo. Y a diferencia del renombre del 4/8, la URL vieja **no quedó rota**:
GitHub redirige con 301 al dominio, conservando la ruta.
...
- **El panel de respuestas salta directo a cualquier paso ya contestado**, y
  ...
  es solo por teclado—.
- **La cita del art. 29 inc. e de la cautelar y las descripciones cruzadas de
  la oposición.** Las dos se cerraron el 6/8 y están contadas arriba. Lo que
  sigue vivo de ellas es la lección, y está en `ESTADO.md`: las validaciones
  comparan números, así que un rótulo que promete un porcentaje puede mentir
  con todas en verde.
- **El flujo hacia atrás arrastraba respuestas de un proceso a otro.** Cerrado
  el 3/8 y cubierto por `retroceso.validation.ts`, que barre los 28.224 cruces
  en cada corrida. Lo que sigue vivo es el invariante —`answers` no es un
  acumulador— y está en `ESTADO.md`.
